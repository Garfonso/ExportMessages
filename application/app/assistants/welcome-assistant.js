configconfigconfigselType//JSLint options:
/*jslint sloppy: true */
/*global log, logGUI, logStatus, $L, Mojo, AppAssistant, PalmCall, config, setTimeout */
...
WelcomeAssistant.prototype.setup = function () {
	...
	this.controller.setupWidget("selType", {
		choices: [ {label: "Export Messages", value: false}, { label: "Export Notes", value: true} ],
		modelProperty: "notes"
	}, config);
	this.controller.setupWidget("ckOnlySMS", { modelProperty: "onlySMS", disabledProperty: "notes" }, config);
	this.controller.setupWidget("txtFilename", { modelProperty: "currentfilename", hintText: $L("Filename of the export"), textCase: Mojo.Widget.steModeLowerCase}, config);
	...
	Mojo.Event.listen(this.controller.get("selType"), Mojo.Event.propertyChange, this.handleTypeSelection);
};
...
WelcomeAssistant.prototype.handleTypeSelection = function (event) {
	if (config.notes) {
		config.fileName = config.currentFilename;
		config.currentFilename = config.filenameNotes;
	} else {
		config.filenameNotes = config.currentFilename;
		config.currentFilename = config.filename;
	}
	this.controller.modelChanged(config, this);
};
